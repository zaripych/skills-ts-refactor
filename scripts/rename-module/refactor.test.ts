...
const setup = ({ fixturePath }: { fixturePath: string }) =>
  setupRefactoring({ importMetaDirname: import.meta.dirname, fixturePath, refactor: renameModule })

describe('renameModule', () => {
  it('renames a directory and rewrites alias and relative importers', async () => {
    await using harness = await setup({ fixturePath: 'fixtures/fixture-rename' })
    ...
    expect(await readFile({ relativePath: 'src/app.ts' })).toContain("from '@/features/invoices'")
    expect(await readFile({ relativePath: 'src/features/invoices/index.ts' })).toContain("from './format'")
    expect(await readFile({ relativePath: 'src/features/invoices/index.ts' })).toContain(
      "from '../../utils/logger'",
    )
  })
  ...
  it('renames a directory referenced from inside via crossing relative import', async () => {
    ...
    expect(await readFile({ relativePath: 'src/app.ts' })).toContain("from '@/helpers/logger'")
    expect(await readFile({ relativePath: 'src/features/billing/index.ts' })).toContain(
      "from '../../helpers/logger'",
    )
  ...
  it('rewrites touched relative imports to absolute with --absolute-imports', async () => {
    ...
    await run({ args: ['src/utils', 'helpers', '--absolute-imports', '@/'], diff: false })

    expect(await readFile({ relativePath: 'src/features/billing/index.ts' })).toContain("from '@/helpers/logger'")
    expect(await readFile({ relativePath: 'src/features/billing/index.ts' })).toContain("from './format'")
    expect(await readFile({ relativePath: 'src/app.ts' })).toContain("from '@/helpers/logger'")
  })
  ...
  it('renames a single file module', async () => {
    ...
    expect(await readFile({ relativePath: 'src/app.ts' })).toContain("from '@/models/account'")
    expect(await readFile({ relativePath: 'src/features/billing/index.ts' })).toContain(
      "from '@/models/account'",
    )
  })
  ...
  it('rewrites alias and relative jest/vi/vitest mock paths', async () => {
    ...
    const fromTests = await readFile({ relativePath: 'src/tests/billing.spec.ts' })
    expect(fromTests).toContain("vi.mock('../features/invoices/createInvoice')")
    expect(fromTests).toContain("vitest.mock('../features/invoices/createInvoice')")
  })
  ...
  it('leaves a within-directory relative mock path unchanged when renaming its directory', async () => {
    ...
    expect(await readFile({ relativePath: 'src/features/invoices/createInvoice.spec.ts' })).toContain(
  ...
  it('rewrites a relative mock path to alias with --absolute-imports', async () => {
    ...
    await run({ args: ['src/features/billing', 'invoices', '--absolute-imports', '@/'], diff: false })

    expect(await readFile({ relativePath: 'src/tests/billing.spec.ts' })).toContain(
      "vi.mock('@/features/invoices/createInvoice')",
    )
  })

  it('refuses to rename a file referenced through an exact path alias', async () => {
    await using harness = await setup({ fixturePath: 'fixtures/fixture-exact-alias' })
    const { run, readFile, readStructure } = harness
    const before = await readFile({ relativePath: 'src/app.ts' })
    const structureBefore = await readStructure()

    await expect(run({ args: ['src/utils/logger.ts', 'log'], diff: false })).rejects.toThrow(
      /exact tsconfig path aliases/,
    )
